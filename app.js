...
const updateActiveSlide = () => {
  document.querySelectorAll(".title").forEach((el, index) => {
    if (index === currentIndex - 1) { // индексы начинаются с 0
...
const updateImages = (index) => {
  const imgSrc = `assets/img${index}.jpg`;
  ...
  gsap.to([imgTop, imgBottom], {
    ...
    ease: "power4.out",
    stagger: 0.15,
    onComplete: trimExessImages,
  });
};
const trimExessImages = () => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  updateImages(1); // Начинаем с первого изображения
  updateActiveSlide();
});